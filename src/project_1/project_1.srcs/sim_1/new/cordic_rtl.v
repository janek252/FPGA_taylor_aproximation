...
always @(posedge clock) begin
    if (reset == 1'b1) begin
        ...
        case(state)
            ...
            S3: begin
                n_x_2 <= -x_base * x_base / FXP_SCALE;
                sin <= x_base;
                state <= S4;
            end
            S4: begin
                x_tmp <= x_base * x_base / FXP_SCALE;
                i <= 0;
                state <= S5;
            end
            S5: begin
            ...
            S6: begin
                sin <= sin + (i_table[i] * x_tmp / FXP_SCALE);
                x_tmp <= x_tmp * n_x_2 / FXP_SCALE;
                i <= i + 1;
                state <= S5;
            end
            S7: begin
